...
#             self.node_attn_weight[self.ntype_list[i]] = nn.Linear(self.out_dim * self.num_attn_head, 1, bias=False)
    ...
    def forward(self, graph, node_feats_dict, node_emb_dict, edge_emb_dict, debug=False):
        """
...
#                     print('SKIPPING NO EDGES')
                ...
                if node_feats_dict.get(dst_type) is not None:
                    t1 = node_feats_dict.get(dst_type)[ :subgraph.number_of_dst_nodes()]
                    t2 = edge_emb_dict[e_type]
                    
                    # Grab POC: applying Linear transformation to make the dim the same as input dimension (i.ie in_dim)
                    X = node_feats_dict.get(dst_type)[ :subgraph.number_of_dst_nodes()].type(th.FloatTensor)
                    X = X.to(self.device)
                    if dst_type in self.ntype_feature_dim_dict:
                        X = self.node_feature_linears[dst_type](X)                        
                    
                    dst_feat = X + edge_emb_dict[e_type]                    
#                     dst_feat = node_feats_dict.get(dst_type)[ :subgraph.number_of_dst_nodes()] + edge_emb_dict[e_type]
                ...
                if node_feats_dict.get(src_type) is not None:
                    ...
                    X = node_feats_dict.get(src_type).type(th.FloatTensor)
                    X = X.to(self.device)
...
#                     src_feat = node_feats_dict.get(src_type) + node_emb_dict[src_type] + edge_emb_dict[e_type]
                else:
                ...
                # expend to 3D
#                 query_dst = query(dst_feat)[:subgraph.num_dst_nodes].view(-1, self.num_attn_head, self.out_dim)
                ...
                # print("In edge {}:{}".format(e_type, edge_heads.shape))
    ...
    def forward(self, graph, node_feats, debug=False):
        ...
        if graph.is_block or True:
            ...
            for src_type, e_type, dst_type in graph.canonical_etypes:
                ...
                c_e_type = (src_type, e_type, dst_type)
#                 print('forward - canonical etype', c_e_type)
                
#                 subgraph = dgl.edge_type_subgraph(graph, [(src_type, e_type, dst_type)])
                subgraph = graph[(src_type, e_type, dst_type)]
...
#                 target_nodes_index = th.nonzero(target_nodes_TF == True)
...
#                 query_dst = query(node_feats[dst_type][:subgraph.number_of_dst_nodes()])
#                 query_dst = query(node_feats[dst_type][:subgraph.number_of_dst_nodes()]).view(-1, self.num_attn_head, self.out_dim)

#                 query_dst = query(node_feats[dst_type]).view(-1, self.num_attn_head, self.out_dim)
...
#         h = th.cat([target_emb, target_feat], dim=1)
...
#             nn.init.constant_(para, 0.)
    ...
    def forward(self, graph, in_node_feats, out_node_feats, debug=False):
        """
        ...
        h_tensors = self.gnn_layers[0](graph[0], in_node_feats, self.node_emb_dict, self.edge_emb_dict)
