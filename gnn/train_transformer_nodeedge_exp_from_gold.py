...
from graphtransformer_models_node_edgefeature_version_exp import graphtransformer_model_paper_mb
...
# def get_edge_feature_dict(graph):
...
def run_inference(model, val_dataloader, node_features, edge_feat_dict, labels, target, device):
    model.train(False)
    ...
    for input_n, output_n, mfgs in val_dataloader:
        ...
        logits = model(mfgs, bt_in_feat, bt_out_feat, edge_feat_dict)
...
#     edge_features, edge_features_dim_dict = get_edge_feature_dict(graph)
    ...
    model = graphtransformer_model_paper_mb(input_dim,
                                  ...
                                  gnn_n_layer,
                                  edge_exc_level=args.edge_exc_level,
                                  target=target,
                                  device=device)
    ...
    for epoch in range(num_epochs):
        ...
        for cnt, (input_n, output_n, mfgs) in enumerate(dataloader):
            ...
            logits = model(mfgs, bt_in_feat, bt_out_feat, edge_features)
        ...
        val_loss, val_acc, _, _ = run_inference(model, val_dataloader, node_features, edge_features, labels, target, device)
...
#         save_model(model, args.model_dir, epoch)
    ...
    test_loss, test_acc, test_gt_list, test_pred_list = run_inference(model, test_dataloader, node_features, edge_features, labels, target, device)
